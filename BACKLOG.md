...
- **CLOSED: Phase 2.5.1 — pool refresh test coverage** (2026-05-12). 9 new tests across 3 files close the Phase 2.5 test gap. `internal/pool/refresh_concurrent_test.go` (50-goroutine concurrency + 401 cooldown + 5xx backoff), `internal/tokenvault/commit_meta_patch_test.go` (metadata merge, stale-generation rejection, malformed-metadata tolerance), `internal/server/refresh_e2e_test.go` (full-stack E2E: vault → pool → refresh → kiroclient sees new Bearer). Concurrency test documents a PRE-EXISTING gap — `RefreshConfig.group` singleflight field exists but is never invoked; in-process coalescing falls through to vault `Reserve` serialization, which surfaces `ErrLockHeld` to losing callers instead of waiting for winner. Tracked separately as P1 "Phase 2.5.2: wire singleflight.Group.Do around refreshOne".
...
- **CLOSED: Upstream 403 with fresh credentials + new endpoint** (2026-05-13, Track-1 c2+c4). Root cause was compounded: (a) `claude-sonnet-4-5` dashed form not in `internal/models/models.go` mapping table → upstream 400 `INVALID_MODEL_ID`; (b) `expires_at` in metadata derived from import time instead of token `addedAt` time → Phase 2.5 proactive refresh never fired → reactive refresh returned same stale token in a loop. Fixed by adding dashed aliases (`claude-sonnet-4-5`, `claude-haiku-4-5`, `claude-opus-4-5`) and by deriving `expires_at = parse(addedAt) + expiresIn`. End-to-end verified: `POST /v1/messages` returns 200. Original entry follows: SURFACED 2026-05-12 live smoke. Symptoms: `/v1/messages` returns 502 (upstream 403 empty body) even with (a) freshly refreshed access_token verified valid via `kiroxy debug-refresh`, (b) migrated `runtime.us-east-1.kiro.dev` endpoint, (c) Phase 2.5 reactive refresh triggers 3x per request. Direct manual curl to `runtime.us-east-1.kiro.dev` with same access_token + profileArn + simpler payload shape returns 200 + valid EventStream SSE including `contextUsageEvent` and `meteringEvent`. Conclusion: kiroxy payload construction diverges from what the new endpoint accepts. Suspect: elaborate history/toolContext/agentTaskType fields that the manual minimal payload does not include. Next steps: (1) capture kiroxy's actual outbound body via the temp KIROXY_TAP, (2) diff against minimal-working curl body field-by-field, (3) identify which field triggers rejection. LoC estimate: 30-100 (likely field name or null-vs-omit issue in reqconv).
- **CLOSED: `added_at + expires_in` vs `expires_at` discrepancy** (2026-05-13, Track-1 c4). `cmd/kiroxy/import_json.go` now calls `deriveExpiresAt(addedAt, expiresIn)` which parses RFC3339 then legacy local-time `2006-01-02T15:04:05`, falling back to `time.Now()`. 5 table-driven subtests. Original entry follows: SURFACED 2026-05-12. `/tmp/dash-preview.db` showed `expires_at` 1h **beyond** what `added_at + expires_in` math predicts. First import at 22:08 claimed expiry at 00:09 (2h later) instead of 23:08 (1h). Impact: Phase 2.5 proactive refresh trigger window is miscalibrated — either fires too early or too late. Needs deterministic test with mocked time + reconciliation between `cmd/kiroxy/import_json.go` (sets `expires_at = time.Now().Unix() + ExpiresIn`) and kiro_login.py output (sets `addedAt` at login time). LoC estimate: 20-50.
...
- **CLOSED: KIROXY_UPSTREAM_URL env var** (2026-05-13, Track-1 c5). `Config.KiroUpstreamURL` parses `KIROXY_UPSTREAM_URL`; `main.go` threads it into `kiroclient.WithBaseURL` on both paths. Tests in `internal/config/config_test.go`. Original entry follows: Phase L mock_kiro cannot be integration-tested through kiroxy without this. Simple baseURL override already exists at `kiroclient.WithBaseURL(...)`; just needs env plumbing in `internal/config/config.go`. LoC: 10-20.

## P0/P1 surfaced by Phase S research (research-v4/, 2026-05-13)

- **CLOSED: P0 SSE keepalive pings during slow thinking blocks** (FAIL-043 in research-v4/FAILURES.md). Shipped — derived from d-kuro/kirocc (Apache 2.0). `internal/respconv/streaming.go::keepaliveLoop` runs in a goroutine launched at `NewSSEWriter`, fires `event: ping\ndata: {"type":"ping"}` every 15s until `done` channel closes or `ctx.Done`. Concurrent-safe via `writeMu`. Original symptoms (claude-code + opencode chunkTimeout abort during Opus 4.7 adaptive thinking) are gone.
- **CLOSED: P1 Wire OpenTelemetry tracing at runtime** (READINESS.md §3.2). Shipped — `cmd/kiroxy/main.go:160-171` calls `tracing.Init(ctx)` when `KIROXY_OTEL_ENABLED=1`, plumbs `otelShutdown` through `awaitShutdown` for graceful flush. OTLP HTTP exporter + W3C propagators ship via the existing `internal/tracing/` package.
...
- **CLOSED: P1 Boot-time vault file mode check** (FAIL-035). Shipped — `internal/tokenvault/vault.go::tightenVaultPerms` runs on every `Open`, checks the SQLite + WAL + SHM sidecars, applies `chmod 0600`, and emits `slog.Warn("tokenvault: chmod 0600 failed", ...)` per file on failure (without aborting startup). Drift from `cp -p` or backup tooling is now reasserted at every boot.
- **CLOSED: P1 Tests for `internal/config`** (READINESS.md §6). Shipped — `internal/config/config_test.go` covers env parsing, default resolution, validation rules, and the `KIROXY_UPSTREAM_URL` plumbing.
